...
{	$requser = $bdd->prepare('SELECT * FROM utilisateur WHERE id = ?');
	...
	$pt = $userinfo['points'];
		...
		$Marv = $bdd->prepare('SELECT * FROM comics WHERE id_Categorie = 2');
	$Marv->execute(array());
...
<body>
	...
	<div class="content-area">
		<div class="wrapper">
			<h2>E-Comics</h2>
			<article>
 <p>Ici vous vous  trouverz dans la section Dc</p>
 <p>Vous trouvererais des classics du monde des comics ainsi que des histoires plus courantes et plus recentes</p>
 <p>Si vous commencez dans le monde des comics Dc : Voici quelques suggestions
 Premierement si vous ne connaissait rien vous pouvais commentcer par les anthologieS </p>
 </article>
...
if (isset($_POST['point'])) 
	{
		...
		{
			$msg = 'Vous n avez pas les points necessaires';
		}
		else
		{
			$update = $bdd->prepare("UPDATE utilisateur set  points = ? WHERE id = ?");
			$update->execute(array($pts,$userinfo['id']));
			header('location:comics.php?id='.$idd);
		}
